...
Esta seção orienta sobre como os microdados do SAEV devem ser baixados, além de apresentar as estruturas de dados e o formato dos arquivos.

## Como baixar os microdados


Para baixar os microdados do SAEV para este ambiente, siga os passos abaixo:
1) Entre no sistema [SAEV](https://saev.abc.br);
2) Faça login;
3) Selecione no menu a opção **Exportar Microdados**;
4) No filtro **Tipo de Microdado**, selecione a opção **Avaliação Normalizada**;
5) Nos demais filtros, selecione as opções conforme a necessidade.



A depender do seu perfil de acesso no sistema, você só poderá baixar os microdados do seu município. Se o seu perfil for Regional, poderá baixar os microdados de todos os municípios que abrangem a sua região. Por fim, se o seu perfil for Estadual, você poderá baixar os microdados de todos os municípios do estado.
...
A seguir, é apresentada em detalhe a estrutura dos arquivos CSV, seus campos, tipos de dados e descrição. Esta informação é útil para a criação das tabelas de banco de dados no DuckDB (banco de dados OLAP).



Os arquivos no formato CSV possuem as seguintes características:
...
3) O conteúdo de alguns campos, como nome do aluno, escola e endereço, pode conter espaços em branco antes e depois do nome. Nestes casos, recomenda-se a exclusão desses espaços no processo de ETL.
...
O arquivo testes.csv contém informações sobre o teste aplicado. A tabela a seguir apresenta as informações sobre o arquivo testes.csv.
...
Este arquivo contém as informações das escolas. A tabela a seguir apresenta informações sobre o arquivo.
...
Este arquivo contém as informações do município. A tabela a seguir apresenta informações sobre os campos do arquivo municipios.csv.
